...
router.get('/cosmos/photos/:start_date/:end_date/:page/:limit', async(req : Request, res : Response) => {
    const start_date : string | string[] = req.params.start_date
    const end_date : string | string[] = req.params.end_date
    const page : number = Number(req.params.page)
    const limit : number = Number(req.params.limit)
    const offset : number = (page - 1) * limit


    if (!isNaN(Date.parse(String(start_date))) && !isNaN(Date.parse(String(end_date)))) {
        try {
            const response  = await axios.get('https://api.nasa.gov/planetary/apod', {
                params: {
                    api_key: "jk4uiZkVB9zdL0f1fhrUAeaBwdK8Bl3hydnL8umn",
            ...
            const paginatedItems = (response.data).slice(offset, offset + limit)
            const nextPage = '/cosmos/photos/' + start_date + "/" + end_date + "/" + String(page + 1) + "/" + String(limit)
            res.json({
                totalPages: Math.ceil(response.length / limit), items: paginatedItems, nextPage: nextPage
            });
